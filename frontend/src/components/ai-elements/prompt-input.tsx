...
> & {
  onSpecialKeyDown?: KeyboardEventHandler<HTMLTextAreaElement>;
};
...
export const PromptInputTextarea = ({
  ...
  const rawValue = controller ? controller.textInput.value : undefined;
  const valueForHighlight =
  ...
  const highlightedText = useMemo(() => {
    ...
    while (match) {
      ...
      const atBoundary = start === 0 || /\s/.test(previousChar);
      ...
      nodes.push(
        <span
          key={`mention-${start}`}
          className="bg-violet-100 text-violet-700 rounded px-1 py-0.5 dark:bg-violet-900/50 dark:text-violet-300"
        >
          {mention}
        </span>,
      );
    ...
    if (lastIndex < value.length) {
      nodes.push(
        <span key={`text-end-${lastIndex}`}>{value.slice(lastIndex)}</span>,
      );
    }
  ...
  return (
    <div className="relative w-full">
      <div
        ...
        ref={overlayRef}
        className={cn(
          "pointer-events-none absolute inset-0 z-0 overflow-hidden px-3 py-3 text-sm whitespace-pre-wrap break-words",
          "text-foreground",
        )}
      >
        {highlightedText}
      ...
      <InputGroupTextarea
        ...
        className={cn(
          "field-sizing-content max-h-48 min-h-16 relative z-10 caret-foreground text-transparent selection:bg-accent selection:text-accent-foreground placeholder:text-muted-foreground",
          className,
        )}
        name="message"
