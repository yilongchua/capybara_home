...
from src.control_plane.models import ControlPlaneSnapshot
...
class ControlPlaneStore:
    ...
    def read(self) -> ControlPlaneSnapshot:
        with self._lock:
    ...
    def write(self, snapshot: ControlPlaneSnapshot) -> None:
        with self._lock:
            self.path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def mutate(self, fn: Callable[[ControlPlaneSnapshot], T]) -> T:
        with self._lock:
            snapshot = self.read()
            result = fn(snapshot)
            self.write(snapshot)
