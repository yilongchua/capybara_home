...
tools:
  - name: web_search
    ...
    max_results: 5
    extract_top_k: 2
    extract_max_chars: 4096
    extract_timeout_seconds: 45
    # Controls concurrent browser contexts. Set to 2 to prevent Out-Of-Memory exhaustion.
    extract_max_workers: 2
```

**Built-in Tools**:
- `web_search` - Search via SearXNG and optionally enrich top URLs with crawl4ai (`extracted_content`)
- `image_search` - Search images (SearXNG images category)
...
- `SEARXNG_BASE_URL` - SearXNG endpoint URL
- `CAPYBARA_HOME_CONFIG_PATH` - Custom config file path
