...
start() {
    ...
    # Set CAPYBARA_HOME_ROOT for provisioner if not already set
    if [ -z "$CAPYBARA_HOME_ROOT" ]; then
        export CAPYBARA_HOME_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting CAPYBARA_HOME_ROOT=$CAPYBARA_HOME_ROOT${NC}"
        echo ""
    fi
...
stop() {
    # CAPYBARA_HOME_ROOT is referenced in docker-compose-dev.yaml; set it before
